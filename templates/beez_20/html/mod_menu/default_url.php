...
if((strpos($item->link, 'Itemid=') === false)){
    $class = $item->anchor_css ? 'class="external-link '.$item->anchor_css. $accesskey_class.'" ' : 'class="external-link'.$accesskey_class.'" ';
} else {
    $class = $item->anchor_css ? 'class="'.$item->anchor_css . $accesskey_class .'" ' : ' ';
}
